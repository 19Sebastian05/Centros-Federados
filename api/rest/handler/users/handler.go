...
import (
	"gcff-auth/internal/ciphers"
	"gcff-auth/internal/password"
	"gcff-auth/pkg/auth/roles_password_policy"
	"gcff-auth/pkg/auth/users"
	...
	"gcff-auth/internal/response"

	"gcff-auth/internal/msgs"
...
func (h *Handler) UpdateUser(c *fiber.Ctx) error {
	...
	m.Password = password.Encrypt(m.Password)
	m.PasswordConfirm = ""
	user, cod, err := serviceUsers.UpdateUser(id, m.Username, m.CodeStudent, m.Dni, m.Names,
		m.LastnameFather, m.LastnameMother, m.Email)
	if err != nil {
		logger.Error.Println("couldn't create user: ", err)
		res.Code, res.Type, res.Msg = msg.GetByCode(cod)
		if cod == 15 {
